...
class Tcp_Client_Function(object):
    def __init__(self):
    ...
    def log_recode(self, log_checkbutton):
        ...
        if log_checkbutton.get():
            try:
                self.f = open("%s_%s_tcp_client.txt" % (self.HOST, self.PORT), 'ab')
            except Exception, e:
    ...
    def clear_data_text(self, data_daxt, send_num_entry, receive_num_entry):
        '对应tcp_client面板>统计框架>清空按钮的函数，用于清空接收和发送的数据，全部置为0'
    ...
    def send_data(self, send_entry_data, send_num_entry, data_text):
        '对应tcp_client面板>发送数据框架>发送按钮的函数，用于向客户端发送带条数据'
        self.s.sendall(send_entry_data)
        # 客户端发送给服务器的数据，同样显示在接收数据的框架里面
    ...
    def sent_data_timing_circulation_1(self, send_entry_data, send_interval, send_frame_button, send_num_entry, data_text):
        ...
        while aa and self.signal:
            ...
            try:
                ...
                self.s.sendall(send_entry_data_all)
                wait_time = 0.0
                send_entry_data_all = ''
            except Exception, e:
    ...
    def send_data_timing_1(self, send_frame_button, send_entry_data, send_interval, send_num_entry, data_text):
        '对应tcp_client面板>发送数据大框架>发送数据小框架1>定时发送按钮的函数，用于向客户端定时发送数据'
        ...
        try:
            if send_frame_button.get() == u'定时发送' and self.signal:
                ...
                if send_interval.get().isdigit():
                    send_data_timing_thread = threading.Thread(target=self.sent_data_timing_circulation_1, args=(send_entry_data, send_interval,  send_frame_button, send_num_entry, data_text))
    ...
    def sent_data_timing_circulation_2(self, send_entry_data, send_interval, send_frame_button, send_num_entry, data_text):
        'sent_data_timing_circulation_2，就是定时发送1函数拉起的线程对应的函数，用于循环形式发送数据'
        # 如果在这个线程里面不需要改变bb的值，那下面的global bb就不需要，同时bb不能在这里进行更改；
        # 在这里将bb置为全局变量，那么就可以在这里对aa进行赋值。
        global bb
        data_text.insert(Tkinter.END, "%s [%s>>server] %d: （定时发送2开始）%s\n" % (time.strftime("%H:%M:%S"), self.s.getsockname(), len(send_entry_data),  send_entry_data))
        sleep_time = float(send_interval.get())/1000
        send_entry_data_all = ''
        wait_time = 0.0
        while bb and self.signal:
            #下面的try，只要是用于当获取选中客户端失败、发送数据失败、插入数据到显示区域失败时；可以将定时按钮重置；同时可以再次定时发送
            try:
                while wait_time < 0.2 and len(send_entry_data_all) < 1400:
                    send_entry_data_all += send_entry_data
    ...
    def send_data_timing_2(self, send_frame_button, send_entry_data, send_interval, send_num_entry, data_text):
        '对应tcp_client面板>发送数据大框架>发送数据小框架1>定时发送按钮的函数，用于向客户端定时发送数据'
        # 这个全局变量bb仅用于发送数据小框架2的定时发送，使用bb来对是否进行定时发送进行判断；bb作用于send_data_timing_2函数和
        # 由send_data_timing_3函数拉起的线程。
        # 点击“定时发送”>bb=1>拉起线程sent_data_timing_circulation_1>循环发送数据”
        # 点击“暂停”>因为这个时候“定时发送”按钮的值为“暂停”，也就是要暂停定时发送>bb=0>因为aa为全局变量，所以send_data_timing_circulation_2
        ...
        try:
            if send_frame_button.get() == u'定时发送' and self.signal:
                bb = 1
                if send_interval.get().isdigit():
                    send_data_timing_thread = threading.Thread(target=self.sent_data_timing_circulation_2, args=(send_entry_data, send_interval,  send_frame_button, send_num_entry, data_text))
                    send_data_timing_thread.start()
                    send_frame_button.set(u'暂停')
                else:
                    print u"请输入正整数"
    ...
    def sent_data_timing_circulation_3(self, send_entry_data, send_interval, send_frame_button, send_num_entry, data_text):
        'sent_data_timing_circulation_3，就是定时发送1函数拉起的线程对应的函数，用于循环形式发送数据'
        # 如果在这个线程里面不需要改变aa的值，那下面的global cc就不需要，同时cc不能在这里进行更改；
    ...
    def send_data_timing_3(self, send_frame_button, send_entry_data, send_interval, send_num_entry, data_text):
        '对应tcp_client面板>发送数据大框架>发送数据小框架3>定时发送按钮的函数，用于向客户端定时发送数据'
        ...
        try:
            if send_frame_button.get() == u'定时发送' and self.signal:
                cc = 1
                if send_interval.get().isdigit():
                    send_data_timing_thread = threading.Thread(target=self.sent_data_timing_circulation_3, args=(send_entry_data, send_interval,  send_frame_button, send_num_entry, data_text))
                    send_data_timing_thread.start()
                    send_frame_button.set(u'暂停')
                else:
                    print u"请输入正整数"
    ...
    def receive_data(self, param_list):
        ...
        while self.signal:
            ...
            # 如果客户端主动断开连接，那么conn.recv可以接收到空的数据，然后中断连接；但如果是服务器想主动断开连接，那这个conn.recv会一直处于等待状态，这个线程就无法回收。
            data = self.s.recv(1024)
            if data != '':
                param_list[0].insert(Tkinter.END, "%s [%s>>server] %d: %s\n" % (time.strftime("%H:%M:%S"), self.s.getsockname(), len(data),  data))
                try:
    ...
    def Connect(self, param_list):
        '对应tcp_client面板>侦听框架>侦听按钮的函数，用于tcp_client的侦听；param_list就是从侦听按钮附带下来的参数的列表'
        ...
        if param_list[11].get() == u'连接':
            try:
                ...
                # 如果这里直接使用self.receive_data，那么就一直等待数据过来，下面的代码就无法走下去；所以采用线程的方式拉起接受数据的函数。
                self.accept_thread = threading.Thread(target=self.receive_data, args=(param_list, ))
